...
use core::{hint::spin_loop, sync::atomic::spin_loop_hint};
...
use esp32_c3::{manager_service, pgns::{
        depth_128267::task_depth_128267,
...
async fn main(spawner: Spawner) {
    ...
    let my_name = 0xFFF5567890ABCFFF;
    let preferred_address = 148;
    let manager = match AddressManager::new(
        can_bus,
        korri_timer,
        my_name,
        preferred_address,
    ...
    {
        Ok(mgr) => {
            println!("Address claimed: {}", mgr.current_address());
            mgr
        }
        Err(_) => {
            println!("Failed to claim an address on the NMEA2000 bus");
            panic!("address claim failed");
        }
    };

    let (_runner, mgr_handle) = manager_service::init_manager(manager);


    // spawner.spawn(led_ticker(led)).unwrap();
    spawner.spawn(test_ticker()).unwrap();
    spawner.spawn(task_depth_128267(mgr_handle)).unwrap();
    spawner.spawn(task_speed_128259(mgr_handle)).unwrap();
    // spawner.spawn(task_engine_127488(manager_mutex)).unwrap();
    // spawner.spawn(task_engine_127489(mgr_handle)).unwrap();
